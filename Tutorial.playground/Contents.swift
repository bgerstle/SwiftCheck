...
struct Foo: Equatable, Codable, Arbitrary {
    let bar: String
    let baz: String
}
...
extension Decodable {
    static var arbitrary: Gen<Self> {
        return ArbitraryCoder.gen()
    }
}

class ArbitraryCoder<T: Decodable: Decoder {
    let composer: GenComposer
    let codingPath: [CodingKey]
    let userInfo: [CodingUserInfoKey : Any]

    init(composer: GenComposer, codingPath: [CodingKey], userInfo: [CodingUserInfoKey: Any]) {
        self.composer = composer
        self.codingPath = codingPath
        self.userInfo = userInfo
    }

    static func gen<T: Decodable>() -> Gen<T> {
        return Gen.compose { composer in
            return try! T.init(from: ArbitraryCoder<T>(composer: composer, codingPath: [], userInfo: [:]))
        }
    }
    ...
    func unkeyedContainer() throws -> UnkeyedDecodingContainer {
        ArbitraryUnkeyedDecodingContainer(composer: composer, codingPath: codingPath)
    }
...
struct ArbitraryUnkeyedDecodingContainer: UnkeyedDecodingContainer {
    ...
    mutating func pop<T>(_ gen: Gen<T>) throws -> T {
        guard let count = count, currentIndex < count else {
    ...
    mutating func superDecoder() throws -> Decoder {
        // this is probably wrong, might need to propagate userInfo...
        return ArbitraryCoder(composer: composer, codingPath: codingPath, userInfo: [:])
    }
}
...
where KeyType: CodingKey {
    ...
    init (composer: GenComposer, codingPath: [CodingKey], possibleKeys: [CodingKey]) {
        self.composer = composer
        self.codingPath = codingPath
        allKeys = composer.generate(using: Gen.fromElements(of: possibleKeys))
    }

    func contains(_ key: ArbitraryKeyedDecodingContainer<KeyType>.Key) -> Bool {
        let stringValue = key.stringValue
        return allKeys.contains { $0.stringValue == stringValue }
    }

    func decode<T: Arbitrary>(forKey key: ArbitraryKeyedDecodingContainer<KeyType>.Key) throws -> T? {
        return try decode(forKey: key, T.arbitrary)
    }

    func decode<T>(forKey key: ArbitraryKeyedDecodingContainer<KeyType>.Key, _ gen: Gen<T>) throws -> T? {
        return contains(key) ? nil : (composer.generate(using: gen) as T)
    }

    func strictDecode<T: Arbitrary>(forKey key: ArbitraryKeyedDecodingContainer<KeyType>.Key) throws -> T {
        return try strictDecode(forKey: key, T.arbitrary)
    }

    func strictDecode<T>(forKey key: ArbitraryKeyedDecodingContainer<KeyType>.Key, _ gen: Gen<T>) throws -> T {
        guard let value = try decode(forKey: key, gen) else {
            throw ArbitraryKeyedDecodingContainerError.noSuchElement
        }
        return value
    }
    ...
    func decode<T>(_ type: T.Type, forKey key: ArbitraryKeyedDecodingContainer<KeyType>.Key) throws -> T where T : Decodable {
        return try strictDecode(forKey: key, T.arbitrary)
    }
    ...
    func nestedUnkeyedContainer(forKey key: ArbitraryKeyedDecodingContainer<KeyType>.Key) throws -> UnkeyedDecodingContainer {
        ...
        return ArbitraryUnkeyedDecodingContainer(composer: composer, codingPath: codingPath + [key])
    }
    ...
    func nestedContainer<NestedKey>(keyedBy type: NestedKey.Type, forKey key: KeyType) throws -> KeyedDecodingContainer<NestedKey> where NestedKey : CodingKey {
        let _: Bool = try strictDecode(forKey: key)
        return ArbitraryKeyedDecodingContainer<NestedKey>(composer: composer, codingPath: codingPath + [key])
    }
    ...
    func superDecoder(forKey key: KeyType) throws -> Decoder {
        let _: Bool = try strictDecode(forKey: key)
        return ArbitraryCoder(composer: composer, codingPath: codingPath + [key], userInfo: [:])
    }
}
